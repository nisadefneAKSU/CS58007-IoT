...
### Find the optimal k for manual KNN using a held-out validation split
def find_best_k_auto(X_train, y_train, k_values=range(1, 11)):
    best_k = None
    best_f1 = -1.0 #0
...
# ---------------------------------------------------------------
def knn_for_aggregate_data_auto(base_path, k=5):
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train, y_test = load_har_aggregate_dataset(base_path)
    best_k = find_best_k_auto(X_train, y_train) # We found the best k by cross-validation
    knn_for_aggregate_data_auto(base_path, k=best_k) 

    user_df = load_har_user_dataset(base_path)     
    knn_per_user_auto(user_df, k=best_k) # We chose the best k by trying k manually for user-specific (individual) data
